...
func fly(myPlane : Aviatrix) {
    ...
    for (index, city) in destinations.enumerated() {
        let distance = myPlane.distanceTo(target: city, currentLocation: myPlane.currentLocation)
...
func refuel(myPlane : Aviatrix) {
    ...
    print("Refueling...")
    print("⛽ Here in \(myPlane.currentLocation), jet fuel costs \(data[plane.currentLocation])!")
    print("⛽ You refueled \(myPlane.refuel()) gallons totaling \((plane.fuelCost()))")
}
...
var plane = Aviatrix(userName: "Mariana")
...
print(" ")
print("Thanks for flying with \(plane.author) airlines!")
